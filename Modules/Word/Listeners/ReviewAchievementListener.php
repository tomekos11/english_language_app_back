...
class ReviewAchievementListener
{
    /**
    ...
    {
        $achievement_array = [];
        $achi = Achievement::where('event_type','=','review')
            ->get();
        $review_count = $event->user->review_counter;
        foreach($achi as $ach){
            if($review_count == $ach->value && !$event->user->achievements()->where('event_type','=','review')->where('value','=',$ach->value)->first()){
